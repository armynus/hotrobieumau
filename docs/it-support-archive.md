...
- Phiếu có ba trạng thái hiển thị: **Chờ tiếp nhận**, **Đang xử lý**, **Đã xử lý**. Phiếu cũ từng ở trạng thái `closed` vẫn được tính vào **Đã xử lý**. Lần đầu phiếu chuyển sang **Đã xử lý**, `completed_at` được cố định. Tệp và bản ghi `PHIEU-{id}.json` được lưu trong `storage/app/private/it-support/NAM YYYY/THANG MM-YYYY/NGAY DD-MM-YYYY/PHIEU-{id}`. Bản ghi gồm nội dung, kết quả, người gửi, tên tệp và lịch sử phản hồi. Phản hồi về sau cập nhật bản ghi này.
